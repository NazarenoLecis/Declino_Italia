...
# PATHS (usa ./)
# =============================================================================
PATH_VA = "./Produzione e valore aggiunto per branca di attività (IT1,92_1225_DF_DCCN_ANA1_1,1.0).xlsx"
PATH_HOURS = "./Monte ore lavorate (base 2021) (IT1,534_1037_DF_DCSC_ORE10_2_7,1.0).xlsx"
...
OUTDIR = "./output_va_vs_hours_base2021"
os.makedirs(OUTDIR, exist_ok=True)

PLOTS_DIR = os.path.join(OUTDIR, "plots_all_sectors")
...
def detect_header_row_for_va(path: str, sheet_name=0, max_rows: int = 80) -> int:
    """
...
def save_unmatched(va_sectors, hours_sectors, mapping, outdir):
    ...
    pd.Series(unmatched_va, name="unmatched_va").to_csv(os.path.join(outdir, "unmatched_va_sectors.csv"), index=False, encoding="utf-8")
    pd.Series(unmatched_h, name="unmatched_hours").to_csv(os.path.join(outdir, "unmatched_hours_sectors.csv"), index=False, encoding="utf-8")

# =============================================================================
# 0) (opzionale) stampa metadata ore (prime righe) per trasparenza
# =============================================================================
def print_hours_metadata(path: str, sheet_name: str, nrows: int = 12):
...
# =============================================================================
def load_va_annual(path: str, sheet_name=0) -> pd.DataFrame:
...
# =============================================================================
def load_hours_quarterly(path: str, sheet_name: str, header: int) -> pd.DataFrame:
...
# =============================================================================
def build_sector_mapping(va_long: pd.DataFrame, hours_a: pd.DataFrame, fuzzy_threshold: int = 92) -> dict:
    ...
    # Fuzzy opzionale per chiavi non matchate esattamente
    # (usa rapidfuzz se disponibile; altrimenti salta)
    try:
...
# =============================================================================
def plot_sector_va_vs_hours(panel: pd.DataFrame, sector_va: str, outdir: str):
    ...
    outpath = os.path.join(outdir, f"va_vs_hours_{slugify(sector_va)}.png")
    fig.savefig(outpath, dpi=200, bbox_inches="tight")
    plt.close(fig)

def plot_sector_productivity(panel: pd.DataFrame, sector_va: str, outdir: str):
    sub = panel[panel["sector_va"] == sector_va].sort_values("year").copy()
    if sub.empty:
        return
    ...
    outpath = os.path.join(outdir, f"prod_idx_{slugify(sector_va)}.png")
    fig.savefig(outpath, dpi=200, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    # Se non ti serve, commenta la riga sotto.
    ...
    va_out = os.path.join(OUTDIR, "va_annual_long.csv")
    hq_out = os.path.join(OUTDIR, "hours_quarterly_long.csv")
    ha_out = os.path.join(OUTDIR, "hours_annualized.csv")
    ...
    # 3) Matching settori
    mapping = build_sector_mapping(va_long, hours_a, fuzzy_threshold=92)
    ...
    mapping_out = os.path.join(OUTDIR, "mapping_va_to_hours.csv")
    ...
    panel_out = os.path.join(OUTDIR, f"panel_va_hours_indices_base{BASE_YEAR}.csv")
    ...
    summary_out = os.path.join(OUTDIR, f"summary_sectors_va_gt_hours_base{BASE_YEAR}.csv")
    ...
    print(f"- Ore lavorate: trimestrale; foglio '{HOURS_SHEET}'. Controlla il metadata stampato sopra per 'Correzione' (destagionalizzati / calendar adjusted).")
