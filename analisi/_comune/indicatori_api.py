...
OUTDIR = BASE_DIR / "output_indicatori"
PLOTS_DIR = OUTDIR / "plots_key"
...
def _year_columns(columns: Iterable[object]) -> list[str]:
    out = []
    for col in columns:
        text = str(col)
...
def fetch_eurostat_productivity_panel(start_year: int = 1995) -> pd.DataFrame:
    ...
    for spec in EUROSTAT_PRODUCTIVITY_SERIES:
        ...
        ycols = _year_columns(raw.columns)
        ...
        long["source_url"] = "https://ec.europa.eu/eurostat/databrowser/view/nama_10_lp_ulc/default/table?lang=en"
...
def add_derived_indicators(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    if not merged.empty:
        ...
        merged["source"] = "World Bank WDI - elaborazione ORA Lab"
        merged["source_url"] = (
    ...
    if not tourism.empty:
        ...
        tourism["source"] = "World Bank WDI - elaborazione ORA Lab"
        tourism["source_url"] = (
...
def add_long_run_transforms(panel: pd.DataFrame, baseline_year: int = 2000, window: int = 10) -> pd.DataFrame:
    ...
    for (indicator_id, country_key), sub in panel.groupby(["indicator_id", "country_key"], sort=False):
        ...
        baseline = _first_from_year(sub, start_year=baseline_year)
...
def build_indicator_panel(refresh: bool = False, start_year: int = 1995) -> pd.DataFrame:
    ...
    panel_path = OUTDIR / "declino_italia_indicator_panel.csv"
    meta_path = OUTDIR / "declino_italia_sources.json"
    ...
    panel = panel.sort_values(["theme", "indicator_label", "country_key", "year"]).reset_index(drop=True)
    panel.to_csv(panel_path, index=False, encoding="utf-8")

    sources = (
        panel[["source", "source_url", "indicator_id", "indicator_label"]]
        .drop_duplicates()
        .sort_values(["source", "indicator_label"])
        .to_dict(orient="records")
    )
    meta_path.write_text(json.dumps(sources, ensure_ascii=False, indent=2), encoding="utf-8")
    return panel


def _first_from_year(df: pd.DataFrame, start_year: int = 2000) -> pd.Series | None:
...
def summarize_decline(panel: pd.DataFrame, baseline_year: int = 2000) -> pd.DataFrame:
    ...
    for indicator_id, sub in panel.groupby("indicator_id", sort=False):
        ...
        base = _first_from_year(italy, start_year=baseline_year)
        last = _latest(italy)
        ...
        for peer in PEER_KEYS:
            peer_last = _latest(sub[(sub["country_key"] == peer) & (sub["year"] <= latest_year)])
...
def save_summary(summary: pd.DataFrame) -> Path:
    OUTDIR.mkdir(exist_ok=True)
    out = OUTDIR / "declino_italia_indicator_summary.csv"
    summary.to_csv(out, index=False, encoding="utf-8")
    return out


def save_transforms(transforms: pd.DataFrame) -> Path:
    OUTDIR.mkdir(exist_ok=True)
    out = OUTDIR / "declino_italia_indicator_transforms.csv"
    transforms.to_csv(out, index=False, encoding="utf-8")
    return out
...
def plot_indicator(panel: pd.DataFrame, indicator_id: str, outdir: Path = PLOTS_DIR) -> Path:
    ...
    ax.legend(ncol=2)
    fig.text(0.01, 0.01, f"Fonte: {source} - elaborazione ORA Lab", ha="left", fontsize=8)
...
def write_snapshot(summary: pd.DataFrame) -> Path:
    OUTDIR.mkdir(exist_ok=True)
    out = OUTDIR / "declino_italia_snapshot.md"
...
def costruisci_pannello_indicatori(refresh: bool = False, start_year: int = 1995) -> pd.DataFrame:
    return build_indicator_panel(refresh=refresh, start_year=start_year)


def riassumi_indicatori(panel: pd.DataFrame, baseline_year: int = 2000) -> pd.DataFrame:
    return summarize_decline(panel, baseline_year=baseline_year)


def salva_grafici_principali(panel: pd.DataFrame) -> list[Path]:
    return save_key_plots(panel)


def esegui_analisi_declino(refresh: bool = False, start_year: int = 1995) -> dict[str, object]:
    return run_decline_analysis(refresh=refresh, start_year=start_year)


def main() -> None:
    result = esegui_analisi_declino(refresh=False)
    ...
    print(f"[SAVED] {result['transforms_path']}")
    print(f"[SAVED] {result['snapshot_path']}")
